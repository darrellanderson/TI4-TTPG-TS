...
export function createQueryParams(
  ...
  if (hasUnits(attacker) || hasUnits(defender)) {
    ...
    window.history.replaceState(
      {},
      "",
      `${location.pathname}${paramsNonEmpty ? "?" : ""}${params}`
    );
  } else {
...
export function applyQueryParams(
  ...
  objectEntries(query).forEach(([key, value]) => {
    if (Array.isArray(value)) {
      console.warn(
        `value of ${key} was unexpected array: ${JSON.stringify(value)}`
      );
      return;
    }
    ...
    if (unitDamagedMatch) {
      ...
      if (isNaN(num)) {
        console.warn(
          `Unit damaged number was not number for ${unit}: ${value}`
        );
        return;
      } else if (side === participant.side) {
...
export function hasSomeQueryParams(
  query: Record<string, string | string[] | undefined>
) {
