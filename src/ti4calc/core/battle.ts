...
export function doBattle(battle: BattleInstance): BattleResult {
  ...
  while (!battleResult) {
    ...
    battle.attacker.onCombatRoundEndBeforeAssign.forEach((effect) => {
      if (canBattleEffectBeUsed(effect, battle.attacker)) {
        effect.onCombatRoundEndBeforeAssign!(
          battle.attacker,
          battle,
          battle.defender,
          effect.name
...
function doSpaceCannon(battle: BattleInstance) {
  if (battle.place === Place.space) {
    const attackerHits = getSpaceCannonHits(
      battle.attacker,
      battle,
      battle.defender
    );
  ...
  const defenderHits = getSpaceCannonHits(
    battle.defender,
    battle,
    battle.attacker
  );
...
function doAfb(battle: BattleInstance) {
  ...
  battle.defender.afbHitsToAssign = getAfbHits(
    battle.attacker,
    battle,
    battle.defender
  );
  battle.attacker.afbHitsToAssign = getAfbHits(
    battle.defender,
    battle,
    battle.attacker
  );
...
function resolveAfbHits(p: ParticipantInstance) {
  while (p.afbHitsToAssign.fighterHitsToAssign > 0) {
    const aliveFighter = p.units.find(
      (u) => u.type === UnitType.fighter && !u.isDestroyed
    );
    if (aliveFighter) {
...
function doParticipantBattleRolls(
  ...
  const friendlyAuras = friendlyUnitTransformEffects.filter(
    (effect) => !!effect.transformUnit
  );
  ...
  const hits = units
    ...
    .map((unit) => {
      ...
      const hitInfo: HitInfo = unit.combat
        ? getHits(unit.combat)
        : { hits: 0, rollInfoList: [] };
...
function resolveParticipantHits(
  ...
  while (hasHitToAssign(p)) {
    ...
    if (p.hitsToAssign.hitsAssignedByEnemy > 0) {
      ...
      const highestWorthNonSustainUnit = getHighestWorthNonSustainUnit(
        p,
        battle.place,
        true
      );
      if (highestWorthNonSustainUnit) {
        ...
        const highestWorthSustainUnit = getHighestWorthSustainUnit(
          p,
          battle.place,
          true
        );
      ...
      const appliedHitToNonFighter = applyHit(
        battle,
        p,
        false,
        isDuringCombat,
...
function logAttack(
  ...
  if (LOG && roll) {
    ...
    if (count === 1 && reroll === 0) {
      ...
      console.log(
        `${p.side} does ${rollType} with ${unit.type} at ${hit}, using ${count} dices`
      );
    } else if (count === 1) {
      console.log(
        `${p.side} does ${rollType} with ${unit.type} at ${hit}, using ${reroll} rerolls`
      );
    } else {
      console.log(
...
function logHits(
  ...
  if (LOG) {
    if (hits.hits === 0 && hits.hitsToNonFighters === 0) {
      ...
      console.log(
        `${p.side} ${rollType} hits ${hits.hitsToNonFighters} to non-fighters.`
      );
    } else {
      console.log(
