...
export const lightrailOrdnance: BattleEffect = {
  ...
  beforeStart: (
    ...
    if (battle.place === Place.ground) {
      ...
      spacedockCount = p.effects[effectName] ?? 0;
    }
...
export const metaliVoidShielding: BattleEffect = {
  ...
  onCombatRoundEndBeforeAssign: (
    ...
    const bestShieldingTarget = getLowestWorthNonSustainUndamagedUnit(
      p,
      battle.place,
      false
    );
    if (bestShieldingTarget && p.hitsToAssign.hits > 0) {
      ...
      logWrapper(
        `${p.side} uses Metali Void Shielding to sustain ${bestShieldingTarget.type}!`
      );
    }
  },
  timesPerRound: 1,
