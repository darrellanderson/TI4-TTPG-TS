...
export const courageousToTheEnd: BattleEffect = {
  ...
  onDeath: (
    ...
    _times(hits.hits, () => {
      const lowestWorthUnit = getLowestWorthUnit(
        otherParticipant,
        battle.place,
        true
      );
...
export const directHit: BattleEffect = {
  ...
  onEnemySustain: (
    ...
    if ((participant.effects[effectName] ?? 0) > 0) {
...
export const experimentalBattlestation: BattleEffect = {
  ...
  beforeStart: (p: ParticipantInstance, battle: BattleInstance) => {
    ...
    const planetUnit = createUnitAndApplyEffects(
      UnitType.other,
      p,
      battle.place,
      modify
...
export const shieldsHolding: BattleEffect = {
  ...
  onCombatRoundEndBeforeAssign: (
    ...
    if (
      ...
      const cancelHitsToNonFighters = Math.min(
        cancel,
        participant.hitsToAssign.hitsToNonFighters
      );
...
export const blitz: BattleEffect = {
  ...
  transformUnit: (u: UnitInstance, _p: ParticipantInstance, place: Place) => {
    if (
      !doesUnitFitPlace(u, place) &&
      u.type !== UnitType.fighter &&
      !u.bombardment
    ) {
      return {
        ...u,
