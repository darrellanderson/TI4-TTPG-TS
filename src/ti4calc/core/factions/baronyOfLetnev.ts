...
export const baronyOfLetnev: BattleEffect[] = [
  ...
  },
  {
    ...
    onCombatRound: (
      ...
      if ((participant.effects[effectName] ?? 0) > 0) {
        const infantryIndex = participant.units.findIndex(
          (u) => u.type === UnitType.infantry
        );
        if (infantryIndex !== -1) {
  ...
  //TODO: Currently just picks the unit with the highest dice count. It could be smarter and include the "cap" on hit-bonus, reroll-value and stuff like that.
  {
    ...
    onStart: (
      ...
      const highestDiceCountUnit = getHighestDiceCountUnit(
        p,
        "combat",
        Place.space
      );
      if (highestDiceCountUnit && highestDiceCountUnit.combat) {
        ...
        const numUniqueUnits = [
          ...new Set(units.map((unit: UnitInstance) => unit.type)),
        ].length;
