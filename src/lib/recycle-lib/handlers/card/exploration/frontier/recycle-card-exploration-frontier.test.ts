...
import { RecycleCardExplorationFrontier } from "./recycle-card-exploration-frontier";

it("recycle", () => {
  const card: Card = new MockCard({
    cardDetails: [
      new MockCardDetails({
        metadata: "card.exploration.frontier:my-source/my-name",
        tags: ["card-exploration-frontier"],
      }),
    ],
  });
  const mat: GameObject = new MockGameObject({
    ...
    snapPoints: [
      new MockSnapPoint({
        tags: ["discard-exploration-frontier", "card-exploration-frontier"],
      }),
    ],
  });
  ...
  const recycle = new RecycleCardExplorationFrontier();
