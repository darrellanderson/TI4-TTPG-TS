...
export const MINOR_FACTIONS_SLICE_MAKEUPS: ReadonlyArray<
  ReadonlyArray<SystemTierType>
> = [
  ["high", "low", "red", "red"],
  ["med", "med", "red", "red"],
];

export const MINOR_FACTIONS_SHAPE: SliceShape = [
  "<0,0,0>", // home system
  "<1,-1,0>", // left
  "<1,0,-1>", // front
  "<0,1,-1>", // right
...
export const MINOR_FACTIONS_SHAPE_ALT: SliceShape = [
  "<0,0,0>", // home system
  "<1,-1,0>", // left
...
export class MinorFactionsDraft implements IDraft {
  public isEnabled(): boolean {
    return true;
  }

  public getDraftName(): string {
    return "Minor Factions Draft";
  }

  public getGenerateSlicesParams(): GenerateSlicesParams {
    return {
      sliceMakeups: MINOR_FACTIONS_SLICE_MAKEUPS,
      sliceShape: MINOR_FACTIONS_SHAPE,
  ...
  public createEmptyDraftState(namespaceId: NamespaceId): DraftState {
    ...
    draftState.setSliceShape(MINOR_FACTIONS_SHAPE);

    if (TI4.config.playerCount === 7) {
      draftState.overrideSliceShape(3, MINOR_FACTIONS_SHAPE_ALT);
    } else if (TI4.config.playerCount === 8) {
      draftState.overrideSliceShape(3, MINOR_FACTIONS_SHAPE_ALT);
      draftState.overrideSliceShape(7, MINOR_FACTIONS_SHAPE_ALT);
    }
