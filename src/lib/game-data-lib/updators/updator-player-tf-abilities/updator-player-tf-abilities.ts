...
import { Ability } from "../../../twilights-fall-lib/twilights-fall/ability";

export class UpdatorPlayerTFAbilities implements IGameDataUpdator {
  ...
  constructor() {
    ...
    OnCardBecameSingletonOrDeck.onSingletonCardCreated.add(
      (card: Card, _player?: Player): void => {
        ...
        if (card instanceof Card && nsid.startsWith("card.tf-ability")) {
          UpdatorPlayerTFAbilities.setTimestamp(card);
        }
      },
    );
  }
  ...
  update(gameData: GameData): void {
    ...
    for (const obj of world.getAllObjects(skipContained)) {
      ...
      if (
        obj instanceof Card &&
        nsid.startsWith("card.tf-ability") &&
        this._cardUtil.isLooseCard(obj, allowFaceDown)
      ) {
        abilityCards.push(obj);
      }
    ...
    if (draftMat) {
      ...
      abilityCards = abilityCards.filter((abilityCard: Card): boolean => {
        return !atop.isAtop(abilityCard.getPosition());
      });
    }

    // Sort cards by creation order.
    abilityCards.sort((a: Card, b: Card): number => {
      const aTimestamp: number = UpdatorPlayerTFAbilities.getTimestamp(a);
      const bTimestamp: number = UpdatorPlayerTFAbilities.getTimestamp(b);
    ...
    abilityCards.forEach((card: Card): void => {
