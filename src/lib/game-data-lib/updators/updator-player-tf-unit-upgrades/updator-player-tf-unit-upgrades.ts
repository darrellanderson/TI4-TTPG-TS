...
import { UnitUpgrade } from "../../../twilights-fall-lib/twilights-fall/unit-upgrade";

export class UpdatorPlayerTFUnitUpgrades implements IGameDataUpdator {
  ...
  constructor() {
    ...
    OnCardBecameSingletonOrDeck.onSingletonCardCreated.add(
      (card: Card, _player?: Player): void => {
        ...
        if (card instanceof Card && nsid.startsWith("card.tf-unit-upgrade")) {
          UpdatorPlayerTFUnitUpgrades.setTimestamp(card);
        }
      },
    );
  }
  ...
  update(gameData: GameData): void {
    ...
    let unitUpgradeCards: Array<Card> = [];
    ...
    for (const obj of world.getAllObjects(skipContained)) {
      ...
      if (
        obj instanceof Card &&
        nsid.startsWith("card.tf-unit-upgrade") &&
        this._cardUtil.isLooseCard(obj, allowFaceDown)
      ) {
        unitUpgradeCards.push(obj);
      }
    ...
    if (draftMat) {
      ...
      unitUpgradeCards = unitUpgradeCards.filter((unitUpgradeCard: Card): boolean => {
        return !atop.isAtop(unitUpgradeCard.getPosition());
      });
    }

    // Sort cards by creation order.
    unitUpgradeCards.sort((a: Card, b: Card): number => {
      const aTimestamp: number = UpdatorPlayerTFUnitUpgrades.getTimestamp(a);
      const bTimestamp: number = UpdatorPlayerTFUnitUpgrades.getTimestamp(b);
    ...
    unitUpgradeCards.forEach((card: Card): void => {
