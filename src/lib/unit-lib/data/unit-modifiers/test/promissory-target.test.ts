...
it("registry", () => {
  const nsid = "card.event:test/promissory-target";
...
it("modifier", () => {
  placeGameObjects({
    self: ["card.event:test/promissory-target"],
    opponent: ["card.promissory:test/promissory-note"],
  });
  const combatRoll: CombatRoll = CombatRoll.createCooked({
    rollType: "groundCombat",
    hex: "<0,0,0>",
  ...
  expect(combatRoll.getUnitModifierNames()).toEqual(["Promissory Target"]);
