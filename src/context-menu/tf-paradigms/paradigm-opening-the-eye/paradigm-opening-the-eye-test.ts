...
import { ParadigmOpeningTheEye } from "./paradigm-opening-the-eye";
import { HexType } from "ttpg-darrell";

it("constructor, init", () => {
  new ParadigmOpeningTheEye().init();
});
...
it("_getZeroPlanetHexes", () => {
  ...
  const paradigmOpeningTheEye: ParadigmOpeningTheEye = new ParadigmOpeningTheEye();
  const zeroPlanetHexes: Set<HexType> =
    paradigmOpeningTheEye._getZeroPlanetHexes();
...
it("_getAlreadyHaveFrontierTokenHexes", () => {
  ...
  const alreadyHaveFrontierTokenHexes: Set<HexType> =
    paradigmOpeningTheEye._getAlreadyHaveFrontierTokenHexes();
...
it("_getShipHexes", () => {
  ...
  const paradigmOpeningTheEye: ParadigmOpeningTheEye = new ParadigmOpeningTheEye();
  const shipHexes: Set<HexType> = paradigmOpeningTheEye._getShipHexes(11);
...
it("_multiverseShift", () => {
  ...
  paradigmOpeningTheEye._openingTheEye(card, 11);

  // Make sure new frontier token placed in the zero-planet system.
  const hexes: Set<HexType> =
    new ParadigmOpeningTheEye()._getAlreadyHaveFrontierTokenHexes();
